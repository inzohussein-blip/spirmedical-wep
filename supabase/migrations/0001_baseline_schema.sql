-- ═══════════════════════════════════════════════════════════════════
-- 0001 — Baseline Schema (Spir Medical)
-- ═══════════════════════════════════════════════════════════════════
-- خط الأساس الكامل: الامتدادات + الأنواع + 88 جدولاً + RLS + الدوال + الـ triggers.
-- ✅ غير مُدمِّر: كل شيء CREATE ... IF NOT EXISTS / CREATE OR REPLACE / DROP POLICY IF EXISTS
--    ثم CREATE — فتطبيقه على قاعدة قائمة لا يمسّ البيانات (no-op).
-- ⚠️ لإعادة بناء بيئة من الصفر (مسح كامل) استخدم supabase/reset/drop_all_public.sql أولاً
--    (خارج مجلد migrations عمداً كي لا يُطبَّق تلقائياً).
-- ملاحظة: قاعدة الإنتاج مبنيّة أصلاً من هذا المخطط؛ هذا الملف مرجع/أساس للبيئات الجديدة.
-- ═══════════════════════════════════════════════════════════════════
