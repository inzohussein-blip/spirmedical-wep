  ...
  return (
    <>
      ...
      {pendingSpecialists !== null && pendingSpecialists > 0 && (
        <Link
          href="/admin44/specialists/pending"
          style={{
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'space-between',
      ...
      <div style={{
        ...
        <StatCard
          ...
          color="#0E5C4D"
          href="/admin44/orders?filter=today"
        />
        <StatCard
          icon="✅"
          label="مكتمل اليوم"
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {(recentSignups ?? []).length === 0 ? (
              ...
              (recentSignups ?? []).map((u) => (
                <Link
                  key={u.id}
                  href={u.role === 'specialist' ? `/admin44/specialists/${u.id}` : `/admin44/patients/${u.id}`}
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: 12,
