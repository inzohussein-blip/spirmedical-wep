...
import {
  Thermometer, Heart, Utensils, Brain,
  ArrowRight, Search, AlertOctagon, AlertTriangle, Clock, CheckCircle2, AlertCircle,
} from 'lucide-react';
...
const CATEGORIES: Array<{ id: string; label: string; icon: LucideIcon }> = [
  { id: 'general',      label: 'عامة',             icon: Thermometer },
  { id: 'chest',        label: 'الصدر',            icon: Heart },
  { id: 'digestive',    label: 'الجهاز الهضمي',     icon: Utensils },
  { id: 'neurological', label: 'الجهاز العصبي',    icon: Brain },
];
            ...
            <div style={{ padding: '0 18px' }}>
              <div className={`recommendation-card recommendation-${recommendation.urgency}`}>
                <div className="recommendation-icon" aria-hidden="true">
                  {recommendation.urgency === 'emergency' ? <AlertOctagon size={32} strokeWidth={2.2} /> :
                   recommendation.urgency === 'urgent' ? <AlertTriangle size={32} strokeWidth={2.2} /> :
                   recommendation.urgency === 'moderate' ? <Clock size={32} strokeWidth={2.2} /> :
              ...
              <div className="risk-cta-section">
                ...
                {recommendation.urgency === 'emergency' && (
                  <Link href="/guest/sos" className="tool-emergency-btn">
                    <AlertOctagon size={16} strokeWidth={2.4} />
                    طوارئ SOS
                  </Link>
                )}
                <button
