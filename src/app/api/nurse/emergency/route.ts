...
export async function POST(request: NextRequest) {
  ...
  // ─── 6. قناةٌ ثانية للتنبيه (مكالمة آلية) — غير منفَّذة ───
  //
  // التنبيه اليوم عبر Push وحده. وهو كافٍ لأنّ السجلّ محفوظٌ في قاعدة
  // البيانات قبل الإرسال (الخطوة ٤)، فلوحةُ العمليات تعرض الحالة حتى لو
  // لم يصل أيّ إشعار — لا يضيع البلاغ.
  //
  // المكالمة الآلية تحتاج مزوّداً (Twilio أو ما يعادله) بمفاتيحَ وحسابٍ
  // مدفوع، فتبقى قراراً تشغيلياً لا نقصاً في الكود.
