    ...
    <>
      ...
      <form method="GET" style={{ background: 'var(--white)', padding: 16, borderRadius: 14, marginBottom: 20, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
        ...
        <div style={{ marginInlineStart: 'auto', display: 'flex', gap: 4 }}>
          <Link href={`/admin/orders?${new URLSearchParams({ ...searchParams, view: 'list' } as Record<string, string>).toString()}`}
          ...
          <Link href={`/admin/orders?${new URLSearchParams({ ...searchParams, view: 'kanban' } as Record<string, string>).toString()}`}
          ...
          {orders.map((o: any) => {
            ...
            return (
              <tr key={o.id} style={{ borderTop: '1px solid var(--line)' }}>
                <td style={td}>
                  <Link href={`/admin/orders/${o.id}`} style={{ color: 'var(--ink)', textDecoration: 'none', fontWeight: 700 }}>
