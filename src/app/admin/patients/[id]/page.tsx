...
export default async function PatientDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <>
      <Link href="/admin/patients" style={{ fontSize: 12, color: 'var(--ink-3)', textDecoration: 'none', marginBottom: 12, display: 'inline-block' }}>
        ← رجوع
      </Link>
        ...
        <div>
          ...
          <div style={{ background: 'var(--white)', borderRadius: 14, padding: 24, marginBottom: 16 }}>
            <div style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
              ...
              <a
                ...
                style={{
                  padding: '10px 16px', background: '#25D366', color: 'var(--white)',
          ...
          <div style={{ background: 'var(--white)', borderRadius: 14, padding: 20 }}>
            ...
            {(appointments ?? []).length === 0 ? (
              <EmptyState icon="📋" title="لا توجد مواعيد" size="sm" variant="plain" />
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {(appointments ?? []).map((a) => (
                  <Link key={a.id} href={`/admin/orders/${a.id}`} style={{
                    ...
                    <span style={{
                      ...
                      background: a.status === 'completed' ? 'var(--emerald-soft)' :
                                  a.status === 'cancelled' ? 'var(--rose-soft)' : 'var(--amber-soft, #FEF7E0)',
...
function Stat({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div style={{ background: 'var(--white)', borderRadius: 14, padding: 16, textAlign: 'center' }}>
