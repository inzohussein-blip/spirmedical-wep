...
export default function PharmaciesAdminClient({ pharmacies, inventoryCounts }: Props) {
  ...
  const handleDelete = async (p: Pharmacy) => {
    const ok = await confirm({
      title: 'حذف الصيدلية',
      message: `هل تريد حذف "${p.name}" نهائياً؟ سيتم حذف كل أدويتها أيضاً. لا يمكن التراجع.`,
      variant: 'danger',
  ...
  return (
    <div>
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ background: 'var(--white)', borderRadius: 12, overflowX: 'auto', border: '1px solid var(--line)' }}>
