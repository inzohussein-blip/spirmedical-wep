...
import { submitNurseRating } from './actions';
...
/**
 * ════════════════════════════════════════════════════════════════════
 * ⭐ V25.44: NurseRatingForm
 * ════════════════════════════════════════════════════════════════════
 * 
 * يظهر بعد اكتمال الزيارة لتقييم الممرض
 *   ✓ تقييم عام (نجمات)
 *   ✓ تقييمات تفصيلية (4 أبعاد)
 *   ✓ تعليق
 *   ✓ recommend toggle
 * ════════════════════════════════════════════════════════════════════
 */

export default function NurseRatingForm({
  appointmentId,
  specialistId,
  specialistName,
  existingRating,
  ...
  const [hygieneRating, setHygieneRating] = useState(existingRating?.hygiene_rating || 0);
  ...
  const [attitudeRating, setAttitudeRating] = useState(existingRating?.attitude_rating || 0);
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      const result = await submitNurseRating({
        appointment_id: appointmentId,
        specialist_id: specialistId,
        rating,
        hygiene_rating: hygieneRating || undefined,
        expertise_rating: expertiseRating || undefined,
        punctuality_rating: punctualityRating || undefined,
        attitude_rating: attitudeRating || undefined,
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'grid', gap: 10, marginBottom: 14 }}>
        <DetailRating label="النظافة والتعقيم" value={hygieneRating} onChange={setHygieneRating} />
        <DetailRating label="المهنية والخبرة" value={expertiseRating} onChange={setExpertiseRating} />
        <DetailRating label="الالتزام بالوقت" value={punctualityRating} onChange={setPunctualityRating} />
        <DetailRating label="حسن المعاملة" value={attitudeRating} onChange={setAttitudeRating} />
      </div>
